...
export function AboutContent({ locale, ctaBook, ctaWhatsapp, ctaTitle }: Props) {
  ...
  return (
    <main>
      {/* Hero */}
      <section ref={heroRef} className="relative overflow-hidden pt-36 pb-28" style={{ background: "linear-gradient(135deg, #0d0820 0%, #1a0d2e 60%, #0d2a35 100%)" }}>
        <div ref={blobA} className="absolute pointer-events-none" style={{ top: "5%", right: "5%", width: 700, height: 700, background: "radial-gradient(circle, rgba(7,118,136,0.18) 0%, transparent 70%)", filter: "blur(80px)", borderRadius: "50%" }} />
        <div ref={blobB} className="absolute pointer-events-none" style={{ bottom: "-10%", left: "0%", width: 500, height: 500, background: "radial-gradient(circle, rgba(76,175,80,0.12) 0%, transparent 70%)", filter: "blur(70px)", borderRadius: "50%" }} />
        <div ref={blobC} className="absolute pointer-events-none" style={{ top: "40%", left: "30%", width: 400, height: 400, background: "radial-gradient(circle, rgba(7,118,136,0.1) 0%, transparent 70%)", filter: "blur(60px)", borderRadius: "50%" }} />
        ...
        <div className="relative max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-16 items-center">
          <div>
            <motion.div className="flex items-center gap-2 mb-5" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
              ...
              <span className="text-xs font-bold uppercase tracking-[0.2em]" style={{ color: "#4caf50" }}>{isAr ? "من نحن" : "About Us"}</span>
            </motion.div>
            ...
            <motion.h1 className="font-black text-white mb-6 leading-tight" style={{ fontSize: "clamp(40px, 5.5vw, 76px)" }} initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
              ...
              <br />
              <span style={{ WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundImage: "linear-gradient(135deg, #077688, #4caf50)", backgroundClip: "text" }}>
          ...
          <motion.div className="relative h-80 lg:h-96 rounded-3xl overflow-hidden" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.2, duration: 0.6 }}>
            <Image
              ...
              unoptimized
            />
            <div className="absolute inset-0" style={{ background: "linear-gradient(135deg, rgba(7,118,136,0.2), transparent)" }} />
            {/* Floating badge */}
            <motion.div
              className="absolute bottom-6 left-6 px-5 py-3 rounded-2xl"
              style={{ background: "rgba(13,8,32,0.85)", backdropFilter: "blur(12px)", border: "1px solid rgba(7,118,136,0.3)" }}
              initial={{ opacity: 0, y: 16 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.6 }}
            >
              <p className="text-xs font-bold" style={{ color: "#4caf50" }}>{isAr ? "جزء من" : "Part of"}</p>
      ...
      {/* Stats bar */}
      <section style={{ background: "linear-gradient(90deg, #077688, #0a8f60)" }} className="py-10">
      ...
      {/* Burjeel Holdings */}
      <section className="py-28" style={{ background: "#f8fafb" }}>
        <div className="max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-20 items-center">
          <motion.div initial={{ opacity: 0, x: -32 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
            <span className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-[0.2em] mb-5 px-4 py-2 rounded-full" style={{ background: "rgba(7,118,136,0.08)", color: "#077688" }}>
              <span className="w-1.5 h-1.5 rounded-full" style={{ background: "#077688" }} />
              {isAr ? "مجموعة برجيل القابضة" : "Burjeel Holdings"}
            </span>
            <h2 className="font-black mb-6 leading-tight" style={{ fontSize: "clamp(28px, 3.5vw, 48px)", color: "#0d0820" }}>
          ...
          <motion.div className="relative h-72 rounded-3xl overflow-hidden" initial={{ opacity: 0, scale: 0.93 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
            <Image
              ...
              unoptimized
            />
            <div className="absolute inset-0 rounded-3xl" style={{ border: "1px solid rgba(7,118,136,0.15)" }} />
          </motion.div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-16">
            <motion.div className="flex items-center justify-center gap-2 mb-4" initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
              ...
              <span className="text-xs font-bold uppercase tracking-[0.2em]" style={{ color: "#4caf50" }}>{isAr ? "قيمنا" : "Our Values"}</span>
            </motion.div>
            <motion.h2 className="font-black mb-4" style={{ fontSize: "clamp(28px, 3.5vw, 48px)", color: "#0d0820" }} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 }}>
            ...
            {values.map((v, i) => {
              ...
              return (
                <motion.div
                  ...
                  style={{ background: "#f8fafb", border: "1px solid rgba(7,118,136,0.08)" }}
                  initial={{ opacity: 0, y: 24 }}
                  whileInView={{ opacity: 1, y: 0 }}
                  viewport={{ once: true }}
                  transition={{ duration: 0.5, delay: i * 0.1 }}
                  whileHover={{ y: -6, boxShadow: "0 20px 50px rgba(7,118,136,0.1)" }}
                >
                  {/* Corner accent */}
                  ...
                  <h3 className="text-lg font-black mb-3" style={{ color: "#0d0820" }}>
      ...
      {/* RehabTech */}
      <section className="py-28" style={{ background: "linear-gradient(180deg, #f8fafb 0%, #e8f5f7 100%)" }}>
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-16">
            <motion.div className="flex items-center justify-center gap-2 mb-4" initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
              <span className="w-1.5 h-1.5 rounded-full" style={{ background: "#4caf50" }} />
              <span className="text-xs font-bold uppercase tracking-[0.2em]" style={{ color: "#4caf50" }}>{isAr ? "تقنياتنا" : "Our Technology"}</span>
            </motion.div>
            <motion.h2 className="font-black mb-4" style={{ fontSize: "clamp(28px, 3.5vw, 48px)", color: "#0d0820" }} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 }}>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {rehabTechnologies.map((tech, i) => (
              <motion.div
                ...
                whileHover={{ y: -8, boxShadow: "0 20px 50px rgba(7,118,136,0.14)" }}
              >
                {tech.image && (
                ...
                <div className="p-6">
                  <h3 className="font-black text-sm mb-2" style={{ color: "#077688" }}>
      ...
      <section className="py-20" style={{ background: "linear-gradient(135deg, #0d0820, #0d2a35)" }}>
        <div className="max-w-4xl mx-auto text-center">
          ...
          <motion.div className="flex flex-wrap gap-4 justify-center" initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.2 }}>
            <Link href={`/${locale}/book/riyadh`} className="px-8 py-4 rounded-full font-bold text-white transition-all hover:scale-105" style={{ background: "linear-gradient(135deg, #077688, #4caf50)" }}>
